...
if [[ ! -f "$FRONTEND_DIST" ]]; then
  ...
  echo "The backend will serve API and docs, but no frontend UI."
fi
...
echo ""
echo "PostGIS:   127.0.0.1:${DATABASE_PORT}/${DATABASE_NAME}"
echo "GeoServer: http://127.0.0.1:${GEOSERVER_PORT:-8080}/geoserver"
echo "Backend:   http://127.0.0.1:8000"
if [[ -f "$FRONTEND_DIST" ]]; then
  echo "Frontend:  http://127.0.0.1:8000 (served by backend)"
fi
